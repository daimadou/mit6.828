...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		extern char end[];
		bfreeoffset = ROUNDUP((char *) end, PGSIZE);
		nextfree = bfreeoffset;
	}
...
mem_init(void)
{
	...
	// Your code goes here:
	uint32_t pagessize = npages * sizeof(struct PageInfo);
	...
	uint32_t envssize = sizeof(struct Env) * NENV;
	envs = (struct Env *) boot_alloc(envssize);
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref != 0 || pp->pp_link != NULL)
		panic("page_free: page %08x is double freed", pp);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	else
	{
		...
		if(create == 0 ||
		   ...
		   (ptep = (pte_t *)page2kva(pp)) == NULL)
		{
			return NULL;
		}
		pp->pp_ref++;
		*pdep = PADDR(ptep) | PTE_U | PTE_P | PTE_W;
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	assert(!(size % PGSIZE) || 
	       cprintf("boot_map_region:: va 0x%x, pa 0x%x, size 0x%x", va, pa, size));
	uintptr_t va_cur = va;
	physaddr_t pa_cur = pa;
	
	int i;
	pte_t *ptep = NULL;
	for(i = 0; i < size/PGSIZE; i++)
	{
		ptep = pgdir_walk(pgdir, (void *)va_cur, 1);
		if(ptep == NULL)
		{
			return;
		}
		
		*ptep = PTE_ADDR(pa_cur) | PTE_P | perm;		
		va_cur += PGSIZE;
                pa_cur += PGSIZE;
	}
...
{
	// LAB 3: Your code here.
	uint32_t size = (uint32_t)ROUNDUP(va + len, PGSIZE);
